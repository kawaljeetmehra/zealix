...
<a class="dropdown-item" href="#" onclick="event.preventDefault(); document.getElementById('logout-form').submit();">
                                ...
                                <div class="table-responsive mt-2">
                                    <table id="add-row" class="display table table-striped table-hover">
                                        ...
                                        <tbody>
                                            ...
                                            {{ $task->Task_Status === 'not started' ? 'btn-secondary' : '' }}
                                            {{ $task->Task_Status === 'in-progress' ? 'btn-warning' : '' }}
                                            {{ $task->Task_Status === 'completed' ? 'btn-success' : '' }}">
                        ...
                        <div class="modal-body">
                            <form id="taskForm" method="POST" action="{{ route('tasks.store') }}">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="date" id="assign_date" name="assign_date" class="form-control"
                                        required>
                                </div>
                                <div class="mb-3">
    ...
    document.getElementById('salesman_id').addEventListener('change', function() {
        const salesmanId = this.value;

        if (salesmanId) {
            fetch(`/tasks/get-tasks-by-salesman/${salesmanId}`)
                .then(response => {
                    if (!response.ok) {
                ...
                .then(data => {
                    ...
                    // Assuming tasks are strings, no id property.
                    data.tasks.forEach(task => {
                        const option = document.createElement('option');
                        option.value = task; // Use task string directly
                        option.textContent = task; // Use task string directly for display
                        taskSelect.appendChild(option);
                    });
                })
                .catch(error => console.error('Error fetching tasks:', error));
        } else {
            // Clear the task dropdown if no salesman is selected
            const taskSelect = document.getElementById('task_id');
            taskSelect.innerHTML = '<option value="">Select Task</option>';
        }
    });
